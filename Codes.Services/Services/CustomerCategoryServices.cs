...
{
    public class CustomerCategoryServices : BaseService<CustomerCategory, CustomerCategoryDto>, ICustomerCategoryServices
    {
        ...
        public async override Task<IResult> AddAsync(CustomerCategoryDto model)
        {
            try
            {
                ...
                var entity = Mapper.Map<CustomerCategory>(model);
        ...
        public async override Task<IResult> UpdateAsync(CustomerCategoryDto model)
        {
            try
            {
                if (_unitOfWork.Repository.IsExists(q => q.Id != model.Id && (q.NameAr == model.NameAr || q.NameEn == model.NameEn)))
        ...
        public async Task<IDataPagging> GetAllPaggedAsync(BaseParam<MainFilter> filter)
        {
            try
            {
                ...
                var data = Mapper.Map<IEnumerable<CustomerCategoryDto>>(query.Item2);
        ...
        static Expression<Func<CustomerCategory, bool>> PredicateBuilderFunction(MainFilter filter)
        {
            var predicate = PredicateBuilder.New<CustomerCategory>(true);
