...
{
    public class TrackServices : BaseService<Track, TrackDto>, ITrackServices
    {
        public TrackServices(IServiceBaseParameter<Track> businessBaseParameter, IHttpContextAccessor httpContextAccessor) : base(businessBaseParameter, httpContextAccessor)
        {
        }
        public async override Task<IResult> AddAsync(TrackDto model)
        {
            try
            {
        ...
        public async override Task<IResult> UpdateAsync(TrackDto model)
        {
            try
            {
                if (_unitOfWork.Repository.IsExists(q => q.Id != model.Id && (q.NameAr == model.NameAr || q.NameEn == model.NameEn)))
        ...
        public async Task<IDataPagging> GetAllPaggedAsync(BaseParam<MainFilter> filter)
        {
            try
            {
                ...
                var data = Mapper.Map<IEnumerable<TrackDto>>(query.Item2);
        ...
        static Expression<Func<Track, bool>> PredicateBuilderFunction(MainFilter filter)
        {
