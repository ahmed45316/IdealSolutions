...
namespace Codes.Services.Services
{
    public class CountryServices : BaseService<Country, CountryDto>, ICountryServices
    {
        ...
        public async override Task<IResult> AddAsync(CountryDto model)
        {
            try
            {
        ...
        public async override Task<IResult> UpdateAsync(CountryDto model)
        {
            try
            {
                if (_unitOfWork.Repository.IsExists(q => q.Id != model.Id && (q.NameAr == model.NameAr || q.NameEn == model.NameEn)))
        ...
        static Expression<Func<Country, bool>> PredicateBuilderFunction(MainFilter filter)
        {
