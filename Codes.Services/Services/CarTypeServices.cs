...
{
    public class CarTypeServices : BaseService<CarType, CarTypeDto>, ICarTypeServices
    {
        public CarTypeServices(IServiceBaseParameter<CarType> businessBaseParameter, IHttpContextAccessor httpContextAccessor) : base(businessBaseParameter, httpContextAccessor)
        {
        }

        public async override Task<IResult> AddAsync(CarTypeDto model)
        {
            try
            {
                ...
                var entity = Mapper.Map<CarType>(model);
        ...
        public async override Task<IResult> UpdateAsync(CarTypeDto model)
        {
            try
            {
                if (_unitOfWork.Repository.IsExists(q => q.Id != model.Id && (q.NameAr == model.NameAr || q.NameEn == model.NameEn)))
        ...
        public async Task<IDataPagging> GetAllPaggedAsync(BaseParam<MainFilter> filter)
        {
            try
            {
                ...
                var data = Mapper.Map<IEnumerable<CarTypeDto>>(query.Item2);
        ...
        static Expression<Func<CarType, bool>> PredicateBuilderFunction(MainFilter filter)
        {
            var predicate = PredicateBuilder.New<CarType>(true);
