...
namespace Codes.Services.Services
{
    public class TaxCategoryServices : BaseService<TaxCategory, TaxCategoryDto>, ITaxCategoryServices
    {
        ...
        public async override Task<IResult> AddAsync(TaxCategoryDto model)
        {
            try
            {
                ...
                var entity = Mapper.Map<TaxCategory>(model);
        ...
        public async override Task<IResult> UpdateAsync(TaxCategoryDto model)
        {
            try
            {
                if (_unitOfWork.Repository.IsExists(q => q.Id != model.Id && (q.NameAr == model.NameAr || q.NameEn == model.NameEn)))
